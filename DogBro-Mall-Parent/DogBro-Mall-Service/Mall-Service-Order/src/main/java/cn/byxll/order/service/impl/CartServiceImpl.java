...
public class CartServiceImpl implements CartService {
    ...
    public Result<Boolean> add(Integer number, Long goodsId, String userName) {
        ...
        if(sku == null) { return new Result<>(false, StatusCode.ERROR, "加入购物车失败"); }
        // 2、查询spu
        Result<Spu> spuResult = spuFeign.findById(sku.getSpuId());
        Spu spu = spuResult.getData();
        if(spu == null) { return new Result<>(false, StatusCode.ERROR, "加入购物车失败"); }
    ...
    private OrderItem createOrderItem(Integer number, Sku sku, Spu spu) {
        ...
        orderItem.setSpuId(orderItem.getId());
